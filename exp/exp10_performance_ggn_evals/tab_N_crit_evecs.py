"""Create LaTeX tables of critical batch sizes for eigen-pair computation."""

from os import makedirs, path
from typing import Dict

from call_run_N_crit_evecs import N_max, configurations, get_output_file
from run_N_crit_evecs import (
    frac_batch_exact,
    frac_batch_mc,
    full_batch_exact,
    full_batch_mc,
...
FIG_DIR = path.join(HEREDIR, "fig", "N_crit", "evecs")
...
def copy():
    """Copy table files to figure directory."""
    for (architecture, device, param_groups) in configurations():
        src = get_tab_savepath(architecture, device, param_groups)
        copy_to_fig(src)
